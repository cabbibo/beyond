...
void main(){
 ...
 /* vec2 lookup = vec2( abs(xAmount*noiseSize) , yAmount* noiseSize);
  ...
  vec2 lookup = vec2( abs( xAmount * noiseSize ) , yAmount * noiseSize / 10. );
  ...
  if( pos.a > 0. && length( d )  < .5){
    ...
    if( usable > .5 ){
      ...
      if( pos.z > .5 ){
        ...
        vec4 audio2 = texture2D( t_audio , vec2( abs( yAmount * 5. ) , 0.)  );
        
        pos.x +=  length(audio2) * .1;
        pos.a -=  1.1;
      ...
      //lonely
        ...
        for( int i = 0; i < 6; i++ ){
          ...
          if( data.z > .5 ){
            ...
            pos.y -= abs( sim ) * 3. * pow( length( audio ) , 1. ) * (1./(abs(xAmount) +.1)) * dT * 60. ;
         //   pos.z += abs( sim );
         //   pos.x += abs( sim );
