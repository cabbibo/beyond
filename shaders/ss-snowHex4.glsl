...
void main(){
  ...
  sim *= hexNoise( pHex , 14. , 3.1 );
  sim *= hexNoise( pHex , 20. , 30.1 );
  sim *= hexNoise( pHex , 45. , 20.1 );
  ...
  if( pos.a > 0. && length( d )  < .5){
    ...
    if( usable > .5 ){
      ...
      if( pos.z > .5 ){
        ...
        pos.x +=  .5 *length( audio2 ); //* (abs(xAmount ) + ( 1. -tDif)) ;//pow( abs( xAmount ) , 10.);/// length( audio );// (1. +  length( audio2 ) *.1 *( 1. / (xAmount+.5)))/2.;
      ...
      //lonely
        ...
        pos.a -= pow( xAmount , 10. ) * 100000000000.;// * .01* pow(length( audio ), 10. );//* .01 + abs( xAmount * xAmount * xAmount  * .0001) *  1.5;

        pos.a -= length( d ) * length( d );
