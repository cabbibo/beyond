...
void main(){
  ...
  if( pos.a > 0. && length( d )  < .5){
    ...
    if( usable > .5 ){
      ...
      // cool slightly
      if( pos.z > .5 ){
          vec4 audio = texture2D( t_audio , vec2( abs( length( fromCenter ) )* tDif , 0. ));

         pos.a -=5.5;

         pos.x +=1. * length(audio);
          ...
          vec4 audio = texture2D( t_audio , vec2(  length( fromCenter )* tDif, 0. ));
        ...
        pos.a -= .1;
         ...
         if(abs( multiplier )< length(audio)/4.){
        ...
        pos.a -= tDif * abs(multiplier) * length( audio )  * length( audio ) * .01;
        ...
        for( int i = 0; i < 6; i++ ){
          ...
          if( data.z > .5 ){
           ...
           // vec4 audio = texture2D( t_audio , vec2( abs(match) , 0. ));
            ...
            float multiplier = snoise( abs(vec2( r * 10., tDif * 3.1 ) ) );
            ...
            pos.y -= 400.* (10./pos.a) * length( audio ) * ( multiplier );// * data.x;

            if( pos.y < 0. ){

              pos.z += .2;
              pos.x += 4.2 * tDif * match ;//length( audio );
