...
const float triHeight = 0.866025405;
$semLookup
$normalFromDepth
...
void main(){
 ...
 // vNorm = normalize( cross( (position - position1) , (position2 - position1) ));
  ...
  float r = mod( position.x / size , 2. );
  float g = mod( position.y / size, 2.);

  vCol = vec3( r   , g , .5 );


  vec3 pos  = (position.xyz  + vec3( -.5  , -.5 , depth * d.r  )) * vec3( triHeight , 1. , 1. ); 
